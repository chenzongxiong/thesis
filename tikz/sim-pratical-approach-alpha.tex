...
\begin{tikzpicture}[ declare function={gamma(\z)=
    ...
    declare function={gammapdf(\x,\k,\theta) = \x^(\k-1)*exp(-\x/\theta) / (\theta^\k*gamma(\k+1)/\k);}
        ...
        width=10cm,height=6cm,
            ...
            ticks=none,
            xlabel={$\alpha$},
            ylabel={$\Gamma(\alpha; k_\alpha, \theta_\alpha)$},
            legend pos=north east
          % legend style={at={(0.17,0.8)},anchor=south west}
        ]
        \addplot [smooth, domain=0:20, black] {160*gammapdf(x,4,2.0)};
        % \addplot[-,color=blue] coordinates {(4,0) (4,14.5)};
        \node[below] (alphai) at (4,0) {$\alpha_i$};
        % \node[above] (Ldeltai) at (4,14.5) {$L(\delta_i)$};
        
        % \node[below] (delta0) at (1, 0) {$\alpha_0$};        
        % \node[below] (delta1) at (2, 0) {$\alpha0$};        
        % \node[below] (deltak) at (18, 0) {$\delta_k$};        

        % \addplot[-,color=blue] coordinates {(1,0) (1,0.5)};
        % \addplot[-,color=blue] coordinates {(2,0) (2,0.5)};
        % \addplot[-,color=blue] coordinates {(18,0) (18,0.5)};
        \addplot[-,color=black] coordinates {(4,0) (4,0.5)};

        \addlegendentry{$\frac{\alpha^{k_\alpha-1} e^{-\alpha/\theta_\alpha} }{\theta_\alpha^{k_\alpha}\Gamma(k_\alpha)}$}
