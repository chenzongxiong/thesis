...
\pgfplotstableread[col sep=comma]{hnn-stock-diff-outputs.csv}\mydata
...
\begin{axis}[
    ...
    ylabel={$b_n - b_{n-1}$},
    xmin=1300,xmax=1700, xlabel={time step, $n$}
    ]
    %% training sets
    ...
    \addplot+[color=blue,mark=., name path=higher] table[x index={0}, y index={13}]{\mydata}; 
    
   % \addplot[blue!40] fill between[of=lower and higher];
    \addlegendentry{$b_n - b_{n-1}$ of ground truth}
    \addlegendentry{$b_n - b_{n-1}$ of HNN predictions}
