...
\begin{tikzpicture}[ declare function={gamma(\z)=
    ...
    declare function={gammapdf(\x,\k,\theta) = \x^(\k-1)*exp(-\x/\theta) / (\theta^\k*gamma(\k+1)/\k);}
        ...
        width=10cm,height=6cm,
            ...
            ticks=none,
            xlabel={$\beta$},
            ylabel={$\Gamma(\beta; k_\beta, \theta_\beta)$},
        ...
        \node[below] (deltai) at (4,0) {$\beta_i$};
        \node[above] (Ldeltai) at (3.4,14.5) {$D_\textbf{1}(\beta_i)$};
        
        \node[below] (delta0) at (1, 0) {$\beta_0$};        
        \node[below] (delta1) at (2, 0) {$\beta_1$};        
        \node[below] (deltak) at (18, 0) {$\beta_k$};        
        ...
        \addlegendentry{$B\frac{\beta^{k_\beta-1} e^{-\beta/\theta_\beta} }{\theta_\beta^{k_\beta}\Gamma(k_\beta)}$}
        \end{axis}
    \end{tikzpicture}
