...
In this chapter, we make HNN compatible with our market model described in \mychapterref{cha:chapter3}. Then, we discuss how to train HNN. Last, we provide a way to forecast the future price by HNN.


\section{Assumptions}
\label{sec:chapter4:assumption}
In reality, we observe a sequence of price $p_1, p_2, ..., p_N$ for a fixed $N > 0$. Based on \citep{dima2014},
we assume that the price $p_{n}$ hysteretically depends on the underlying noise $b_n$, with $b_n$ being a random walk, what we describe in the previous chapter. Using the notation
...
Based on \citep{dima2014} again, the underlying noise $b_n$ can be expressed as a hysteretic operator depending on the observed prices $p_n$, i.e.,
...
the likelihood of $\mathcal{P}$. Since $\mathcal{P}$ is the deterministic function of a random variable
...
causality property, hence $\mathcal{J(P)}$ is a triangular matrix
\begin{equation}
J(\mathcal{P}) = 
\Scale[1.3]{
...
Thus, maximizing the log-likelihood of \(p(\mathcal{P})\) is equivalent to minimize negative log-likelihood $\mathcal{L}_{ML}$ as following:
\begin{equation}
\begin{aligned}
\mathcal{L}_{ML} = -\ln p(\mathcal{P}) &= \sum_{n=1}^{N} \left(\frac{(b_n-b_{n-1}-\mu_{b})^2}{2 \sigma_{b}^2} + \frac{1}{2} \ln 2 \pi \sigma^2_{b} - \ln \left|\frac{\partial b_n}{\partial p_n}\right|\right) \\ 
&\thicksim \sum_{n=1}^{N} \left(\frac{(b_n-b_{n-1}-\mu_{b})^2}{\sigma_{b}^2} + \ln \sigma^2_{b} - \ln \left(\frac{\partial b_n}{\partial p_n}\right)^2 \right) \\
...
% \section{Dynamics of $\mu_b, \sigma_b, w$}
% \begin{equation}
%     \mu^{new}_b = \mu_b - \lambda_1 \frac{\partial L}{\partial \mu_b}
%                 = \frac{\lambda_1 N + \sigma^2_b}{\sigma^2_b} \mu_b - \frac{\lambda_1}{\sigma^2_b}(b_N-b_0) 
% \end{equation}
% Fixed $\lambda_1, N, \sigma_b, b_0, b_N$, we obtain estimated $\mu^{est}_{b}$
% \begin{equation}
...
% \begin{equation}
%       \sigma^{new}_b = \sigma_b - \lambda_2 \frac{\partial L}{\partial \sigma_b}, \quad\quad
...
% % In \mychapterref{cha:chapter2}, we provide a practical approach to train HNN using $\mathcal{L}_{SE}$ as loss function. In order to make loss function $\mathcal{L}_{ML}$ also work, we fix $\sigma_b = 1$ and reformulate it as: (\mytodo{later we will show that the $\sigma_b$ only matters the magnitude of the distribution of $\mathcal{B}$, or the trend. In our prediction, we only can the trend, not the exactly value.})
% % \begin{equation}
% %     \mathcal{L}_{ML} = \sum^{N}_{n=1} (b_n - b_{n-1} - \mu_b)^2 - \sum^{N}_{n=1} \ln \left(\frac{\partial b_n}{\partial p_n} \right)^2 
% % \end{equation}
% % % Since we cannot observe the real outputs $b_1, \ldots, b_N$ in the market, we assume that the difference between $b_n - b_{n-1} \thicksim \mathcal{N}(\mu_b, \sigma_b)$ i
...
% % % Set $d_n = b_n - b_{n-1}$, 
% % The same is true when one estimates both $\mu_b$ and $\sigma_b$ via maximizing the log-likelihood of a noraml distribution, or, equivalently, minimizing the loss 
% % % \begin{equation}
% % %     \mathcal{L}_{ML}(\mathcal{D}, \mu_b, \sigma_b) = \frac{1}{N}\sum^N_{n=1} \left( \frac{(d_n - \mu_b)^2}{\sigma^2_b} + 2 \ln \sigma_b \right)
% % % \end{equation}

% % % It is well known that the outliers essentially influence the estimate of the mean $\mu$ if one uses the standard error loss

% % % The same is true when one estimates both mean $\mu_b$ and precision $\sigma_b$ via maximizing the log-likelihood of a normal distribution, or, equivalently, minimizing the loss
% % % \begin{equation}
% % %     \mathcal{L}_{SE}(\mathcal{Y}, \mathcal{\hat{Y}}) = \frac{1}{N} \sum^{N}_{n=1}(\hat{y}_i-y_i)^2
% % % \end{equation}

% % The reason is that, in both cases, the derivatives of the loss functions $\mathcal{L}_{SE}$ and $\mathcal{L}_{ML}$ with respect to $\mu_b$ are proportional to $d_n - \mu_b$, while the derivative of $\mathcal{L}_{ML}(\mathcal{D}, \mu_b, \sigma_b)$ with respect to $\sigma_b$ contains even $(d_n - \mu_b)^2$. It turns out that the 
...
\begin{enumerate}
    ...
    : At moment $N$, we already observed price $p_N$. We can set next price $\hat{p}_{N+1} = p_{N} + \Delta$ and iterate all possible $\Delta$ until the condition $b_{N+1} = G(\hat{p}_{N+1}, W_b)$ satisfied. 
