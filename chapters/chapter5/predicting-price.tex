\subsection{Predicting price}
To distinguish difference predicting performance among different methods. We consider the following three approaches. 

\textbf{Methods.} The first approach is to predict the next price $\hat{p}_i$ as the previous price $p_{i-1}$, and we call it native method. The second method is using LSTM cell to build the neural network and the architecture is shown below. The last one is using the methods described in \mysectionref{sec:chapter4:predicting_price}. %we calculate the RMSE as the following algorithm
% \begin{algorithm}[H]
% \SetAlgoLined
...
\textbf{Measure.} To show the fine performance of the predictions, we divide inputs ${p_n}$ and outputs ${b_n}$ into grids and generate the difference  $|\hat{p}_i - p_i|$ at each grid as following, 
\begin{equation}
\begin{aligned}
...
\begin{figure}[ht!]
    ...
    \caption{The consecutive predicted price given by HNN and LSTM networks. \myfigref{fig:chapter5:predicting-price-156-price} and \myfigref{fig:chapter5:predicting-price-157-price} are corresponding to \myfigref{fig:chapter5:predicting-price-156} and \myfigref{fig:chapter5:predicting-price-157} individually.}
