...
% \end{equation}
 ...
 First, we derive \textit{play networks} based on elementary nonlinear function $\Phi(x)$ defined in this thesis. Then we use \textit{play networks} to generate \textit{generalized Prandtl-Ishlinskii (PI) networks}. In the end, we provide a method on how to train a hysteretic network. 
...
The $k$th play operator is given by \citep[p. 9]{krejci1996hysteresis}\footnote{Note that we use the play operator in a slightly different way from the original one described in \citep[p. 9]{krejci1996hysteresis}. Originally, the play operator contains multiple different horizontal lines (see \myfigref{fig:chapter1:play}) and each horizontal line has determined direction. However, each play operator in our implementation has only one horizontal line which is bi-direction (\myfigref{fig:chapter2:play-by-phi}).}:
...
\begin{figure}[htb!]
    ...
    \caption{(\myfigref{fig:chapter2:phi}) The graph of $\Phi(x)$. (\myfigref{fig:chapter2:play-by-phi}) shows the graph of \textit{play}. There are three different \mydelete{hysteretic loops} \myupdate{dynamics}  with different initial states generated from $\Phi(x)$.}
...
\begin{figure}
    ...
    \caption{The neural network cell represents \myformularef{eqn:chapter2:play-operator}. The arrows indicate how data flows in the cell, and the number of $w^k, 1, -1$ beside the arrow line is the weights of this cell. Note that only parameter $w_k$ is learnable in this network since parameters $-1, +1$ are fixed.}
...
\begin{definition}
If a \mydelete{netwrk} \myupdate{network} $\mathcal{G}$ \mydelete{network} is the linear combination of generalized play networks $\mathcal{P}$, we call this network generalized Prandtl-Ishlinskii network or generalized PI network. If the length of the input sequence is m, we call PI networks as $m$-unfolded PI networks.
\end{definition}

% \mytodo{add generalized play networks}

A $\mathcal{G}$ network consists of $K$ $\mathcal{P}$ networks and the initial states of $\mathcal{G}$ network is denoted as follows,
% $p_0$. 
...
\section{Training a hysteretic network \myupdate{given targets (labels)}}\label{sec:chapter2:training-pi-network}
...
\begin{remark}
For a hysteretic process trained by HNN, it will catch up with the \mydelete{trace} \myupdate{dynamics} after finite time steps even though the initial state is incorrect. (see \mysectionref{sec:chapter5:initial-states})
\end{remark}
