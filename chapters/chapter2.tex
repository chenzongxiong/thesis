...
% \begin{equation}
% \min_{W_b} \mathcal{L}_{SE}
% \end{equation}

First, we derive \textit{play networks} based on an elementary nonlinear function $\Phi(x)$ defined in this thesis. Then we use \textit{play networks} to generate \textit{generalized Prandtl-Ishlinskii (PI) networks}. In the end, we provide a method on how to train a hysteretic network. 

\section{Play networks\label{sec:chapter2:play-network}}

Consider $K > 0$ \textit{play} operators. Each of them maps an initial state $p_{0}^{k} \in \mathbb{R} $ and an input sequence $x_1, x_2, \ldots$ to an output sequence $p_{1}^{k}, p_{2}^{k}, \ldots \ $, i.e.,
\begin{equation*}
  p_{0}^{k}, (x_1, x_2, \ldots) \mapsto (p_{1}^{k}, p_{2}^{k}, \ldots), k = 1, \ldots, K
\end{equation*}

The $k$th play operator is given by \citep{kwame2017dynamics},
...
\begin{figure}[htb!]
    ...
    \subfloat[$\text{Play(x)}$]{
        \input{./tikz/play-by-phi-1}
        \label{fig:chapter2:play-by-phi}
    }
    \caption{(\myfigref{fig:chapter2:phi}) The graph of $\Phi(x)$. (\myfigref{fig:chapter2:play-by-phi}) shows the graph of \textit{play}. There are three different \myupdate{dynamics}  with different initial states generated from \myformularef{eqn:chapter2:play-operator}.}
...
\begin{figure}
    ...
    \caption[The representation of \myformularef{eqn:chapter2:play-operator} by neural network cell.]{The neural network cell represents \myformularef{eqn:chapter2:play-operator}. Data flows into the network cells in the direction of arrows, and $w^k, 1, -1$ beside the arrow lines are the weights of this cell. Note that only parameter $w_k$ is learnable in this network since parameters $-1, +1$ are fixed.}
...
If a network represents \myformularef{eqn:chapter2:play-operator}, we call this network a \textsl{play network} $\mathcal{P}$. If we apply nonlinear function $f$ on this network to create a new network $f(\mathcal{P})$, we call $f(\mathcal{P})$ network generalized play network. 
\end{definition}
...
\begin{definition}
If a \myupdate{network} $\mathcal{G}$ is the linear combination of generalized play networks $f(\mathcal{P})$, we call this network generalized Prandtl-Ishlinskii network or generalized PI network. If the length of the input sequence is $m$, we call PI networks as $m$-unfolded PI networks.
\end{definition}

We consider a network $\mathcal{G}$ \myupdate{consisting} of $K$ generalized play networks $f(\mathcal{P})$ and the initial states of $\mathcal{G}$ network is denoted as follows,
...
\begin{equation}\label{eqn:chapter2:pi-contains-one-play}
    y_{n} = \sum_{k=1}^{K} \sum_{s=1}^{S} \hat{\theta}^{k}_{s} p^k_n + \hat{\theta}^{k}_{0} = \sum_{k=1}^{K} \sum_{s=1}^{S} \hat{\theta}^{k}_{s} f_s(\mathcal{P}(x_n, p^k_{n-1}, w^k)) + \hat{\theta}^{k}_{0}, \quad n = 1,2,\ldots
\end{equation}
where $\hat{\theta}^{k}_{s},\hat{\theta}^{k}_{0}$ are parameters, $p^k_n$ is given by \myformularef{eqn:chapter2:play-operator} and $f_s$ is a nonlinear function ($\tanh$ in \myfigref{fig:chapter1:nn-play}). It corresponds to a layer of plays or a \textit{PI network}, see \myfigref{fig:chapter1:nn-pi}.
...
% \section{General hysteretic networks}
% A general network may consist of several \textit{play} layers (and perhaps standard layers) (see \myfigref{fig:chapter1:nn-pi}). We call such a network \textit{hysteretic}, and denote
% \begin{equation}
%     b_n = G(X_n, W)
% \end{equation}
% where $X_n = (x_, \ldots, x_n)$ and $W$ is a vector of all weights of the network.

\section{Training a hysteretic network \myupdate{given targets}}\label{sec:chapter2:training-pi-network}
...
\item Training the $m$-unfolded network: We use adam optimizer \citep{kingma2014adam} to train the $m$-unfolded \textit{PI network}
  ...
  \end{equation*}
  with the inputs $\mathbf{\tilde{x}_1}, \ldots, \mathbf{\tilde{x}_M}$, the true targets $\mathbf{y_1}, \ldots, \mathbf{y_M}$, where
  \begin{equation*}
    \begin{aligned}
    ...
    \end{aligned}
  \end{equation*}
and mean squared error as loss function $\mathcal{L}_{SE}$, where 
\begin{equation*}
    \mathcal{L}_{SE, i} = (F_m(\mathbf{\tilde{x_i}}) - \mathbf{y_i})^2, \quad i = 1,2,\ldots 
\end{equation*}

\item Updating weights $W$:  $W^{new} := W - \nabla_{W} \mathcal{L}_{SE, i}$.
...
\begin{remark}
For a hysteretic process trained by HNN, it will catch up with the \myupdate{trajectory} after finite time steps even though the initial state is incorrect (see \mysectionref{sec:chapter5:initial-states}).
\end{remark}
