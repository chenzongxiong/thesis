...
\begin{figure}[htb!]
    ...
    \caption{(\myfigref{fig:chapter2:phi}) The graph of $\Phi(x)$. (\myfigref{fig:chapter2:play-by-phi}) shows the graph of \textit{play}. There are three different hysteretic loops with different initial states generated from $\Phi(x)$.}
...
\begin{definition}
If a network $\mathcal{G}$ network is the linear combination of generalized play networks, we call this network generalized Prandtl-Ishlinskii network or generalized PI network. If the length of the input sequence is m, we call PI networks as $m$-unfolded PI networks.
\end{definition}
...
A general netwrk may consist of several \textit{play} layers (and perhaps standard layers). We call such a network \textit{hysteretic}, and denote
\begin{equation}
    b_n = G(X_n, W)
\end{equation}
where $X_n = (x_, \ldots, x_n)$ and $W$ is a vector of all weights of the network.

\section{Training a hysteretic network\label{sec:chapter2:training-pi-network}}
...
\begin{remark}
For a hysteretic process trained by HNN, it will catch up with the trace after finite time steps even though the initial state is incorrect. (see \mysectionref{sec:chapter5:initial-states})
\end{remark}
